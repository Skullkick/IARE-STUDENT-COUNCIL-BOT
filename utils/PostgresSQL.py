...
USER= "uc2i842e34b0tq" 
PASSWORD= "pc1fcb8f8285bda85054067377e2da447f607a1a2f8334f02bd877e79c771511e"
DATABASE = "dfcj3kudrcff00"
...
# #    Store or update core team information in the PostgreSQL database. 
...
#                         tasks = COALESCE($5, tasks) 
#                     WHERE core_member_index = $1 
#                 ''', core_team_index, core_name, core_member_name, core_member_chat_id,  
#                     json.dumps(tasks) if tasks else None) 
#             else: 
#                 raise ValueError("Core team index does not exist for update.") 
#         else: 
...
# async def create_core_team_table():
#     """Create the core_team table if it doesn't already exist."""
#     connection = await connect_pg_database()
#     try:
#         await connection.execute('''
#             CREATE TABLE IF NOT EXISTS core_team (
...
#         ''')
#     except Exception as e:
#         print(f"Error while creating core_team table: {e}")
#     finally:
#         await connection.close()
...
async def test_permissions():
    ...
    print("Initializing permissions for chat_id:", test_chat_id)
    await initialize_permissions(test_chat_id)

    print("Retrieving permissions for verification...")
    connection = await connect_pg_database()
    try:
        result = await connection.fetchrow('SELECT * FROM permissions WHERE chat_id = $1', test_chat_id)
        print("Initial Permissions:", result)
    ...
    print("Updating permissions for chat_id:", test_chat_id)
    await set_permissions(
        chat_id=test_chat_id,
        events=True,
        clubs=True,
        edit_clubs="Edit club permissions",
        edit_events="Edit event permissions",
        proposal_form="Proposal form data",
        flyer="Flyer data",
        core_team=True,
        core_team_edit="Core team edit permissions",
        council_admin=True,
        enforcement_team=True,
        photos_drive_link="http://photos.example.com",
        reporter_details="Reporter details",
        report_upload="Upload report link",
        list_of_participants="List of participants"
    )

    print("Retrieving updated permissions for verification...")
    connection = await connect_pg_database()
    try:
        updated_result = await connection.fetchrow('SELECT * FROM permissions WHERE chat_id = $1', test_chat_id)
        print("Updated Permissions:", updated_result)

    except Exception as e:
        print(f"An error occurred while retrieving updated permissions: {e}")
    finally:
        await connection.close()
